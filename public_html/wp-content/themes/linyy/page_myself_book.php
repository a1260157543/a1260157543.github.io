<?php
/*
Template Name: Page myself_book
*/
?>
<?php get_header(); ?>

                <div class="ace-paper-stock">
                    <main class="ace-paper clearfix">

                        <div class="ace-paper-cont clear-mrg">
						
						<!-- START: PAGE CONTENT -->
						<header class="page-header padd-box">
							<h1 class="title-lg text-upper">生 活 动 态</h1>
						</header>
						    <center><div class="pf-filter padd-box">
											<!--<a class="btn btn-sh btn-primary" href="?page_id=348" >全部</a>-->
											<a class="btn btn-sh btn-primary" href="?page_id=543" >随口说说</a>&nbsp;&nbsp;
											<a class="btn btn-sh btn-success" href="?page_id=545" >书籍</a>
							</div></center>
	    <div class="padd-box">						
		<section class="section clear-mrg">
<div class="padd-box-sm clear-mrg">					
<?php
	$limit = get_option('posts_per_page');
	$paged = (get_query_var('paged')) ? get_query_var('paged') : 1;
	query_posts('category_name=book-myself&showposts=' . $limit=6 . '&paged=' . $paged);
	$wp_query->is_archive = true; $wp_query->is_home = false;
?>
<?php while (have_posts()) : the_post(); ?>
                <div class="ref-box brd-btm hreview">
                    <div class="ref-avatar">
                        <a href="<?php the_permalink(); ?>"><img src="<?php bloginfo('template_url'); ?>/img/uploads/avatar/book.jpg" alt=""  height="54" width="54"></a>
                    </div>

                    <div class="ref-info">
                        <div class="ref-author">
                            <a href="<?php the_permalink(); ?>"><strong><?php the_title(); ?></strong></a>
                            <span><?php the_time('Y年n月j日') ?>&nbsp;|&nbsp;<a href="<?php $category = get_the_category(); $category_id= get_cat_id($category[0]->cat_name); echo '?cat='.$category_id; ?>" title="<?php $category = get_the_category(); echo $category[0]->cat_name; ?>"><?php $category = get_the_category();
echo $category[0]->cat_name;?></a></span>
                        </div>
                            <p>
								<a href="<?php the_permalink(); ?>"><?php the_excerpt(); ?></a>
                            </p>
                    </div>
					<hr>
                </div><!-- .ref-box -->
			<?php endwhile; ?>

            </div><!-- .padd-box-sm -->
        </section><!-- .section -->
		
</div>			
						<div class="wp_nav"><div class="page_navi"><?php par_pagenavi(9); ?></div>  </div>
						<!--<div class="wp_nav"><?php echo wp_nav(); ?></div>-->
						    
						<!-- END: PAGE CONTENT -->
						
                </div><!-- .ace-paper-cont -->
            </main><!-- .ace-paper -->
        </div><!-- .ace-paper-stock -->

        </div><!-- .ace-container -->
    </div><!-- #ace-content -->

	<?php get_sidebar(); ?>
<?php get_footer(); ?>